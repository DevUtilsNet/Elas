...
namespace DevUtils.Elas.Tasks.Core.Common
{
	static class WinNT
	{
		...
		private static readonly Dictionary<string, int> SubLanguages = new Dictionary<string, int>
		{
		...
		public static int ParseLanguage(string text)
		{
			int ret;

			if (Languages.TryGetValue(text, out ret))
			{
				return ret;
			}
		...
		public static int ParseSubLanguage(string text)
		{
			int ret;

			if (SubLanguages.TryGetValue(text, out ret))
			{
				return ret;
			}
