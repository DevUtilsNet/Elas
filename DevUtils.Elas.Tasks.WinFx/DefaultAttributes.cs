...
namespace DevUtils.Elas.Tasks.WinFx
{
	...
	internal static class DefaultAttributes
	{
		...
		internal static LocalizabilityAttribute GetDefaultAttribute(object type)
		{
			...
			{
				var predefinedAttribute = DefinedAttributes[type];
			...
			if (targetType != null && targetType.IsValueType)
			{
				...
				// we use this default.
				var attribute = new LocalizabilityAttribute(LocalizationCategory.Inherit) { Modifiability = Modifiability.Unmodifiable };
				return attribute;
			}
